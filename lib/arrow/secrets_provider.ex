defmodule Arrow.SecretsProvider do
  @moduledoc """
  ...
  defp load_prefix(config, prefix, opts) when is_binary(prefix) do
    ...
    |> update_config(
      ...
      fn key ->
        [
          arrow: [
            {ArrowWeb.Endpoint, [secret_key_base: key]}
          ]
        ]
      end,
      opts
    ...
    |> update_config(
      prefix <> "-arrow-auth-secret",
      fn key ->
        [
          arrow: [
            {ArrowWeb.AuthManager, [secret_key: key]}
          ]
        ]
      end,
      opts
    ...
    |> update_config(
      prefix <> "-cognito-client-secret",
      fn secret ->
        [
          ueberauth: [
            {Ueberauth.Strategy.Cognito, [client_secret: secret]}
          ]
        ]
      end,
      opts
