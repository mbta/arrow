defmodule Arrow.Disruption.Revision do
  @moduledoc """
  The insert-only data model that underlies a Disruption. Defines the actual dates, times, and/or
  specific trips to which one or more Adjustments are applied.
  """
  ...
  schema "disruption_revisions" do
    ...
    has_many :days_of_week, DayOfWeek, foreign_key: :disruption_revision_id
    has_many :exceptions, Exception, foreign_key: :disruption_revision_id
    has_many :trip_short_names, TripShortName, foreign_key: :disruption_revision_id
  ...
  @doc false
  @spec changeset_for_create(t(), map(), [Adjustment.t()], DateTime.t()) ::
          Ecto.Changeset.t()
  def changeset_for_create(data \\ %__MODULE__{}, attrs, adjustments, current_time) do
  ...
  def changeset_for_update(data, attrs, current_time) do
    today = DateTime.to_date(current_time)
  ...
  def changeset_for_delete(data, current_time) do
    today = DateTime.to_date(current_time)
  ...
  defp changeset(data, attrs, today) do
    data
