...
const haversineDistance = (pos: [number, number], otherPos: [number, number]) => {
  // radius in miles
  ...
  const arc = Math.cos(
    pos[0] * Math.PI / 180) * Math.cos(otherPos[0] * Math.PI / 180)
    ...
    >
      {existingShuttleStops && (
        <LayersControl.Overlay name={shuttleLayerName}>
          <LayerGroup>
            {existingShuttleStops
              .filter(shuttleStop => haversineDistance([shuttleStop.stop_lat, shuttleStop.stop_lon], [selectedStop.stop_lat, selectedStop.stop_lon]) <= MAX_RADIUS_FOR_STOPS_MILES)
              .map(
                (s, idx) =>
                  s.stop_lat &&
                  s.stop_lon && (<Marker
      ...
      {existingBusStops && (
        <LayersControl.Overlay name={busLayerName}>
          <LayerGroup>
            {existingBusStops
              .filter(gtfsStop => haversineDistance([gtfsStop.lat, gtfsStop.lon], [selectedStop.stop_lat, selectedStop.stop_lon]) <= MAX_RADIUS_FOR_STOPS_MILES)
              .map(
              (s, idx) =>
                s.lat &&
                s.lon && (
