defmodule ArrowWeb.ShuttleControllerTest do
  ...
  describe "create shuttle" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      conn = get(conn, ~p"/shuttles/#{id}")
      assert html_response(conn, 200) =~ "Shuttle #{id}"
    end
  ...
  describe "update shuttle" do
    ...
    test "redirects when data is valid", %{conn: conn, shuttle: shuttle} do
      conn = put(conn, ~p"/shuttles/#{shuttle}", shuttle: @update_attrs)
      assert redirected_to(conn) == ~p"/shuttles/#{shuttle}"
  ...
  describe "delete shuttle" do
    setup [:create_shuttle]

    test "deletes chosen shuttle", %{conn: conn, shuttle: shuttle} do
      conn = delete(conn, ~p"/shuttles/#{shuttle}")
      assert redirected_to(conn) == ~p"/shuttles"

      assert_error_sent 404, fn ->
        get(conn, ~p"/shuttles/#{shuttle}")
      end
    end
  end
