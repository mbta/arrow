defmodule ArrowWeb.ShuttleRouteStopsControllerTest do
  ...
  @create_attrs %{direction_id: :"0", stop_id: "some stop_id", stop_sequence: 42, time_to_next_stop: "120.5"}
  ...
  describe "edit shuttle_route_stops" do
    ...
    test "renders form for editing chosen shuttle_route_stops", %{conn: conn, shuttle_route_stops: shuttle_route_stops} do
  ...
  describe "update shuttle_route_stops" do
    ...
    test "redirects when data is valid", %{conn: conn, shuttle_route_stops: shuttle_route_stops} do
      conn = put(conn, ~p"/shuttle_route_stops/#{shuttle_route_stops}", shuttle_route_stops: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, shuttle_route_stops: shuttle_route_stops} do
      conn = put(conn, ~p"/shuttle_route_stops/#{shuttle_route_stops}", shuttle_route_stops: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Shuttle route stops"
    end
  end
  ...
  describe "delete shuttle_route_stops" do
    ...
    test "deletes chosen shuttle_route_stops", %{conn: conn, shuttle_route_stops: shuttle_route_stops} do
